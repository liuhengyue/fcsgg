"""
Same with `detectron2/utils/analysis`, but replace `Instances` with `SceneGraph`.

Modified to be compatible with Detectron2 based on
https://github.com/facebookresearch/detectron2/blob/main/detectron2/utils/analysis.py
"""
...
def _flatten_to_tuple(outputs):
    ...
    if isinstance(outputs, torch.Tensor):
        ...
        for _, v in outputs.items():
            result.extend(_flatten_to_tuple(v))
    elif isinstance(outputs, SceneGraph):
        # hard to trace integral
        fields = outputs.get_fields()
        fields.pop('rel_inds')
        fields.pop('rel_scores')
        result.extend(_flatten_to_tuple(fields))
...
def _wrapper_count_operators(
    ...
    class WrapModel(nn.Module):
        ...
        def forward(self, image):
            ...
            # the outputs is a list of `Dict` {'scene_graph': SceneGraph, 'instances':Instances}
            # we only gather the scene_graph field in case double counting
            outputs = [output['scene_graph'] for output in outputs]
