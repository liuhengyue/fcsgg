...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, norm="SyncBN"):
        ...
        self.bn1 = get_norm(norm, planes, momentum=BN_MOMENTUM)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes)
        self.bn2 = get_norm(norm, planes, momentum=BN_MOMENTUM)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, norm="SyncBN"):
        ...
        self.bn3 = get_norm(norm, planes * self.expansion, momentum=BN_MOMENTUM)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
    ...
    def _make_one_branch(self, branch_index, block, num_blocks, num_channels, stride=1):
        ...
        if (
            ...
            downsample = nn.Sequential(
                ...
                get_norm(self.norm, num_channels[branch_index] * block.expansion, momentum=BN_MOMENTUM),
            )
                ...
                if j > i:
                    fuse_layer.append(
                        nn.Sequential(
                            ...
                            get_norm(self.norm, num_inchannels[i], momentum=BN_MOMENTUM),
                        )
                    )
                elif j == i:
                        ...
                        if k == i - j - 1:
                            ...
                            conv3x3s.append(
                                nn.Sequential(
                                    ...
                                    get_norm(self.norm, num_outchannels_conv3x3, momentum=BN_MOMENTUM),
                                )
                            )
                        else:
...
class PoseHigherResolutionNet(Backbone):
    ...
    def __init__(self, cfg):
        ...
        self.bn1 = get_norm(self.norm, 64, momentum=BN_MOMENTUM)
        self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1, bias=False)
        self.bn2 = get_norm(self.norm, 64, momentum=BN_MOMENTUM)
        self.relu = nn.ReLU(inplace=True)
                ...
                if num_channels_cur_layer[i] != num_channels_pre_layer[i]:
                    transition_layers.append(
                        nn.Sequential(
                            ...
                            get_norm(self.norm, num_channels_cur_layer[i], momentum=BN_MOMENTUM),
                ...
                for j in range(i + 1 - num_branches_pre):
                    ...
                    conv3x3s.append(
                        nn.Sequential(
                            ...
                            get_norm(self.norm, outchannels, momentum=BN_MOMENTUM),
                            nn.ReLU(inplace=True),
                        )
                    )
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                get_norm(self.norm, planes * block.expansion, momentum=BN_MOMENTUM),
            )
